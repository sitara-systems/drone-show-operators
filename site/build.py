#!/usr/bin/env python3
"""Static site build for The Fleet Operator Directory.
...
content/pages/<slug>.html, each:
    ...
    ---
    title: Page Title | The Fleet Operator Directory
...
def build(out_dir: Path) -> list[dict]:
    # export_data.build_substitutions() is this record's only schema-specific
    # hook: it normalizes data/*.csv, writes any machine-readable output
    # (JSON/CSV) into assets/data/, and returns a {"[[MARKER]]": "html"} dict
    # substituted into content pages below -- same technique as The Exhibit
    # Cost Record's browse tables (full data server-rendered, so crawlers and
    # no-JS visitors see it complete), generalized since each R-track record
    # has its own table shape.
    substitutions = export_data.build_substitutions()
